...
void Pipeline::setObjectPos(const Vector3f& Pos)
{
    m_Object.Pos = Pos;
}

Vector3f Pipeline::getObjectPos() const
{
    return m_Object.Pos;
}

void Pipeline::setObjectAngle(const float Angle)
{
...
const Matrix4f& Pipeline::GetObjectTrans()
{
    ...
    ObjectScaleTrans.InitScaleTransform(m_Object.Scale.x, m_Object.Scale.y, m_Object.Scale.z);
    ObjectTranslationTrans.InitTranslationTransform(m_Object.Pos.x, m_Object.Pos.y, m_Object.Pos.z);
    ObjectRotationTrans.InitRotateTransform(0.f, m_Object.Angle, 0.f);
    m_ObjectTransformation = ObjectTranslationTrans * ObjectRotationTrans * ObjectScaleTrans;
...
void Pipeline::MoveCamera(const Vector3f& delPos)
{
    m_camera.Pos += delPos;
}
...
float Pipeline::getFOV()
{
    return m_persProjInfo.FOV;
}
