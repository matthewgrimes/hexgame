...
bool Mesh::RenderWithOutline(GLfloat red, GLfloat green, GLfloat blue,
                             GLfloat thickness) {
...
void Mesh::LoadFromFile(const std::string &fileName) { LoadMesh(fileName); }
...
bool Mesh::LoadMesh(const std::string &Filename) {
  ...
  const aiScene *m_pScene = m_Importer.ReadFile(
...
bool Mesh::InitFromScene(const aiScene *pScene, const std::string &Filename) {
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(Tangents[0]) * Tangents.size(),
...
bool Mesh::InitMaterials(const aiScene *pScene, const std::string &Filename) {
  ...
  for (unsigned int i = 0; i < pScene->mNumMaterials; i++) {
    ...
    if (m_Textures[i] == "") {
      ResourceManager::getSingleton()->addTexture(
...
void Mesh::Render(bool withOutline) {
  ...
  glUniformMatrix4fv(
      glGetUniformLocation(ShaderManager::getInstance()->getProgram(), "gWVP"),
      1, GL_TRUE, (const GLfloat *)Pipeline::getInstance()->GetWVPTrans());
  glUniformMatrix4fv(
      glGetUniformLocation(ShaderManager::getInstance()->getProgram(),
                           "gObject"),
      1, GL_TRUE, (const GLfloat *)Pipeline::getInstance()->GetObjectTrans());
  ...
  for (unsigned int i = 0; i < m_Entries.size(); i++) {
    ...
    if (isTextureOverridden()) {
      ResourceManager::getSingleton()
          ->getTexture(overrideTexture)
          ->Bind(GL_TEXTURE0);
    } else if (MaterialIndex < m_Textures.size()) {
      ResourceManager::getSingleton()
          ->getTexture(m_Textures[MaterialIndex])
          ->Bind(GL_TEXTURE0);
    }

    if (hasNormalMap()) {
      glUniform1i(glGetUniformLocation(
                      ShaderManager::getInstance()->getProgram(), "bumpMap"),
                  1);
      ResourceManager::getSingleton()->getTexture(normalMap)->Bind(GL_TEXTURE1);
    }

    glDrawElementsBaseVertex(
        GL_TRIANGLES, m_Entries[i].NumIndices, GL_UNSIGNED_INT,
...
void Mesh::clearOverrideTexture() { overrideTexture = std::string(""); }
...
bool Mesh::hasNormalMap() { return (normalMap != std::string("")); }
