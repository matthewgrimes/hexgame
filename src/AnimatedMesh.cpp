...
AnimatedMesh::VertexBoneData::VertexBoneData()
{
    Reset();
}
...
bool AnimatedMesh::LoadMesh(const std::string& Filename)
{
    ...
    m_pScene = m_Importer.ReadFile(Filename.c_str(), aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_CalcTangentSpace | aiProcess_FlipUVs);
...
{
    glUniform1i( glGetUniformLocation(ShaderManager::getInstance()->getProgram(),"skeletalAnimation"), true);
    Mesh::Render(withOutline);
    glUniform1i( glGetUniformLocation(ShaderManager::getInstance()->getProgram(),"skeletalAnimation"), false);
}
bool AnimatedMesh::RenderWithOutline(GLfloat red, GLfloat green, GLfloat blue, GLfloat thickness)
{
...
bool AnimatedMesh::InitFromScene(const aiScene* pScene, const std::string& Filename)
{
    ...
    for (unsigned int i = 0; i < m_Entries.size(); i++)
    {
        ...
        InitMesh(i, paiMesh, Positions, Normals, TexCoords, Indices, Tangents, Bones);
    }
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(Tangents[0]) * Tangents.size(), &Tangents[0],
                 GL_STATIC_DRAW);
    ...
    glVertexAttribPointer(BONE_WEIGHT_LOCATION, 4, GL_FLOAT, GL_FALSE, sizeof(VertexBoneData), (const GLvoid*)(4 * NUM_BONES_PER_VERTEX));
...
void AnimatedMesh::InitMesh(unsigned int MeshIndex, const aiMesh* paiMesh, std::vector<Vector3f>& Positions,
                            std::vector<Vector3f>& Normals,
...
void AnimatedMesh::BoneTransform(float TimeInSeconds, std::vector<Matrix4f>& Transforms)
{
    Matrix4f Identity;
...
void AnimatedMesh::setBoneTransform(unsigned int i,const Matrix4f& Transform)
{
    ...
    glUniformMatrix4fv(glGetUniformLocation(ShaderManager::getInstance()->getProgram(),Name), 1, GL_TRUE, (const GLfloat*)Transform.m);
}
