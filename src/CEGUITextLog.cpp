...
void CEGUITextLog::Init()
{
    ...
    CEGUI::Window* textLogRoot = CEGUI::WindowManager::getSingletonPtr()->loadLayoutFromFile("TextLog.layout");

    textLog = static_cast<CEGUI::Listbox*>(textLogRoot->getChild("TextLog"));

    CEGUI::System::getSingleton().getDefaultGUIContext().getRootWindow()->addChild(textLog);
}

void CEGUITextLog::addText(const std::string& newText, textLogColor color)
{
    CEGUI::Colour ceguiColor = CEGUI::Colour(color.R, color.G, color.B, color.Alpha);
...
void CEGUITextLog::Hide()
{
    textLog->hide();
}

void CEGUITextLog::Show()
{
    textLog->show();
}
