...
void MovementPhase::HandleMouseClick()
{
    ...
    {
        state->activeCharacter->executeMove( state->currentMap.mainCursor.getHexI(), state->currentMap.mainCursor.getHexJ() );
...
{
    if (state->activeCharacter->getMoved() == true && state->activeCharacter->moveList.empty())
    {
