...
{
    rootWindow = CEGUI::WindowManager::getSingleton().loadLayoutFromFile("CharacterInfo.layout");
...
void CharacterInfoBox::registerCharacter(Character* character)
{
    ...
    output = "HP: " + std::to_string(m_character->getHitPoints()) + " / " + std::to_string(m_character->getMaxHitPoints());
    addText(output);
    ...
    output = "Range: " + std::to_string(m_character->getCurrentWeapon()->getMinRange()) + "-" + std::to_string(m_character->getCurrentWeapon()->getMaxRange());
    addText(output);

    output = "Damage: " + std::to_string(m_character->getCurrentWeapon()->getDamage());
    addText(output);
...
{
    static_cast<CEGUI::Listbox*>(rootWindow->getChild("Listbox"))->addItem( new CEGUI::ListboxTextItem(text) );
}
