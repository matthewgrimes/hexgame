...
void SDLDisplay::setupDraw() { shaderManager.ClearBuffers(); }

/** \brief Finish drawing frame
 *
 *  Call at end of drawing to swap draw and screen buffers.
 */
void SDLDisplay::finishDraw() { SDL_GL_SwapWindow(window); }
...
void SDLDisplay::finishPickingRender() { shaderManager.DisablePicking(); }
...
void SDLDisplay::initSDL(const SDLWindowInfo windowInfo) {
  ...
  window = SDL_CreateWindow(windowInfo.name, SDL_WINDOWPOS_UNDEFINED,
                            SDL_WINDOWPOS_UNDEFINED, windowInfo.width,
