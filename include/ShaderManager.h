...
public:
  ...
  virtual void CompileShaders(char const *vs, char const *gs, char const *fs,
                              std::string name);
  ...
  GLuint getProgram() const { return mCurrentProgram; }
...
protected:
  void AddShader(GLuint ShaderProgram, std::string pShaderText,
                 GLenum ShaderType);
  void Setup(GLuint &mProgram, const char *pVSFileName, const char *pGSFileName,
