...
{
    rootWindow->getChild("TurnMenu")->hide();
}

void MapEditorState::Resume()
{
    rootWindow->getChild("TurnMenu")->show();
}
...
void MapEditorState::HandleEvents(CGameEngine* game)
{
    ...
    while ( SDL_PollEvent(&event) )
    {
        ...
        if (event.type == SDL_MOUSEBUTTONDOWN)
        {
            ...
            {
                currentMap.setCurrentData(cI,cJ,currentLayer, (int)(currentLayer == 0) );
            }
        }
...
void MapEditorState::Update(CGameEngine* game)
{
    ...
    if ( CEGUI::System::getSingleton().getDefaultGUIContext().getWindowContainingMouse()->getName() == "TextLog" )
    {
        currentMap.mainCursor.Deactivate();
    }
}
