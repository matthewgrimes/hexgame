...
struct Vector2f {
  ...
  Vector2f operator*(const float r) { return Vector2f(x * r, y * r); }
  Vector2f &operator+=(const Vector2f &r) {
    x += r.x;
    y += r.y;
    return *this;
  }
  bool operator!=(const Vector2f &r) { return (x != r.x || y != r.y); }
  bool operator==(const Vector2f &r) { return (x == r.x && y == r.y); }
};
...
struct Vector4f {
  ...
  void Print() const { printf("(%.02f, %.02f, %.02f, %.02f)", x, y, z, w); }
};
...
public:
  ...
  inline Matrix4f operator*(const float Right) const {
    Matrix4f Ret;
    for (unsigned int i = 0; i < 4; i++) {
      for (unsigned int j = 0; j < 4; j++) {
