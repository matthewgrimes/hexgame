...
void AttackPhase::HandleMouseClick()
{
    ...
    if ( floor(state->currentMap.getCursorData(cI,cJ)) == 1)
    {
        ...
        for ( auto character : state->mCharacters )
        {
            ...
            if ( state->activeCharacter->getCurrentAttack()->checkRange(cI,cJ,character->getI(),character->getJ()) &&
                    abs (state->currentMap.getHeight(cI,cJ) - character->getLevel() ) < 2 )
            {
                target = character;
...
void AttackPhase::Update()
{
    ...
    state->activeCharacter->setAngle( getAngle(state->activeCharacter->getI(), state->activeCharacter->getJ(), state->currentMap.mainCursor.getHexI(), state->currentMap.mainCursor.getHexJ()));
...
void AttackPhase::Activate()
{
    ...
    state->currentMap.mainCursor.setRadius( state->activeCharacter->getCurrentAttack()->getAOE() );
