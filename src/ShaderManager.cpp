...
void OgldevFileError(const char *pFileName, unsigned int line,
                     std::string pFileError) {
#ifdef WIN32
  char msg[1000];
  snprintf(msg, sizeof(msg), "%s:%d: unable to open file `%s`", pFileName, line,
           pFileError.c_str());
...
void ShaderManager::AddShader(GLuint ShaderProgram, std::string pShaderText,
                              GLenum ShaderType) {
...
void ShaderManager::DisablePicking() { mPickingFBO.DisableWriting(); }
...
void ShaderManager::CompileShaders(char const *fileName, std::string name) {
  ...
  CompileShaders(vsFileName.c_str(), gsFileName.c_str(), fsFileName.c_str(),
                 name);
}

void ShaderManager::CompileShaders(char const *vsFileName,
...
void ShaderManager::Setup(GLuint &mProgram, const char *pVSFileName,
                          const char *pGSFileName, const char *pFSFileName) {
...
void ShaderManager::initFBO(const unsigned int width,
...
PickingFBO::PixelInfo ShaderManager::readFBO(const unsigned int x,
...
void ShaderManager::setUniform(const Matrix4f &uniform, std::string name) {
  glUniformMatrix4fv(glGetUniformLocation(getProgram(), name.c_str()), 1,
                     GL_TRUE, (const GLfloat *)uniform);
}
void ShaderManager::setUniform(const Vector3f &uniform, std::string name) {
  glUniform3f(glGetUniformLocation(getProgram(), name.c_str()), uniform.x,
              uniform.y, uniform.z);
}
