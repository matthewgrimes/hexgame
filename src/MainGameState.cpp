...
void MainGameState::Init()
{
    ...
    if ( file.is_open() )
    {
        ServiceLocator::getTextLog().addText("Loading savefile.", textLogColor(0.f,1.f,1.f));
    ...
    ServiceLocator::getGUISystem().subscribeFunction("TurnMenu", "MoveButton", [this]()
    {
        this->movementPhase.Activate();
    });
    ServiceLocator::getGUISystem().subscribeFunction("TurnMenu", "AttackButton", [this]()
    {
        this->activeCharacter->prepareWeaponAttack();
        this->attackPhase.Activate();
    });
    ServiceLocator::getGUISystem().subscribeFunction("TurnMenu", "TechniqueButton",
            [this]()
    {
    ...
    ServiceLocator::getGUISystem().subscribeFunction("TurnMenu", "BackButton", []()
    {
        ServiceLocator::getGUISystem().hideWindow("TurnMenu");
    });
    ServiceLocator::getGUISystem().subscribeFunction("TurnMenu", "EndTurnButton", [this]()
    {
        this->facingPhase.Activate();
    });
...
void MainGameState::HandleEvents(CGameEngine* game)
{
    ...
    while ( SDL_PollEvent(&event) )
    {
        ...
        if ( event.type == SDL_MOUSEBUTTONDOWN || event.type == SDL_MOUSEBUTTONUP || event.type == SDL_MOUSEWHEEL )
        {
...
{
    std::sort( mCharacters.begin(), mCharacters.end(), Actor::turnOrderCompareFunction);
    if ( !turnOrder.empty() )
    {
    ...
    if ( turnOrder.empty() )
    {
        ...
        std::sort( turnOrder.begin(), turnOrder.end(), Actor::turnOrderCompareFunction);

        /* keep delays from growing too big */
        int smallestCountdown = turnOrder.front()->getTurnCountdown();
        for ( auto character : mCharacters )
        {
            character->setTurnCountdown( character->getTurnCountdown() - smallestCountdown );
        }
        ...
        //{
        //printf("In %u turn(s): %s - %f\n",i,turnOrder[i]->getName().c_str(), turnOrder[i]->getTurnCountdown());
        //}
    ...
    if ( ServiceLocator::getGUISystem().getNameOfWindowContainingMouse() == "TextLog" )
    {
        currentMap.mainCursor.Deactivate();
    }
    if ( activeCharacter->getAI() )
...
void MainGameState::Draw(CGameEngine* game)
{
    ...
    for (auto actor : mCharacters)
    {
        glUniform1i(glGetUniformLocation(ShaderManager::getInstance()->getProgram(), "gI"), actor->getHexCoordIJ().getI() + (actor->getHexCoordIJ().getJ() - actor->getHexCoordIJ().getJ() % 2) / 2);
        glUniform1i(glGetUniformLocation(ShaderManager::getInstance()->getProgram(), "gJ"), actor->getHexCoordIJ().getJ());
        glUniform1i(glGetUniformLocation(ShaderManager::getInstance()->getProgram(), "gLayer"), actor->getLevel()+1);
        actor->Render();
    }
...
void MainGameState::drawPortraits()
{
    ...
    for (unsigned int i = 0; i < mCharacters.size(); i++)
    {
        ...
        {
            glUniform1i(glGetUniformLocation(ShaderManager::getInstance()->getProgram(), "gI"), mCharacters[mCharacters.size()-1-i]->getHexCoordIJ().getI() + (mCharacters[mCharacters.size()-1-i]->getHexCoordIJ().getJ() - mCharacters[mCharacters.size()-1-i]->getHexCoordIJ().getJ() % 2) / 2);
            glUniform1i(glGetUniformLocation(ShaderManager::getInstance()->getProgram(), "gJ"), mCharacters[mCharacters.size()-1-i]->getHexCoordIJ().getJ());
            glUniform1i(glGetUniformLocation(ShaderManager::getInstance()->getProgram(), "gLayer"), mCharacters[mCharacters.size()-1-i]->getLevel()+1);

            vertOffset += .1f * (currentMap.mainCursor.getHexCoordIJ() == mCharacters[mCharacters.size() - 1 - i]->getHexCoordIJ());
            Pipeline::getInstance()->setObjectPos(Vector3f(j*2.5f + offset, 0.f, 9.7f - vertOffset));
...
void MainGameState::resetToIdle()
{
    idlePhase.Activate();
}
