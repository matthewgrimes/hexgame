...
void LightingSetup::Init() { m_LightingTechnique.Init(); }
...
void LightingSetupFileLoader::parseLine(const std::string &field,
  ...
  if (field == "spotlights") {
    ...
    if (specs[1] == "diffuse") {
      m_LSetup->m_PointLights[ParseInt(specs[0])].DiffuseIntensity =
          ParseFloat(specs[2]);
    } else if (specs[1] == "color") {
      m_LSetup->m_PointLights[ParseInt(specs[0])].Color = Vector3f(
          ParseFloat(specs[2]), ParseFloat(specs[3]), ParseFloat(specs[4]));
    } else if (specs[1] == "position") {
      m_LSetup->m_PointLights[ParseInt(specs[0])].Position = Vector3f(
          ParseFloat(specs[2]), ParseFloat(specs[3]), ParseFloat(specs[4]));

    } else if (specs[1] == "attenuationlinear") {
      m_LSetup->m_PointLights[ParseInt(specs[0])].Attenuation.Linear =
          ParseFloat(specs[2]);
    }
    ...
    if (specs[1] == "diffuse") {
      m_LSetup->m_SpotLights[ParseInt(specs[0])].DiffuseIntensity =
          ParseFloat(specs[2]);
    } else if (specs[1] == "color") {
      m_LSetup->m_SpotLights[ParseInt(specs[0])].Color = Vector3f(
          ParseFloat(specs[2]), ParseFloat(specs[3]), ParseFloat(specs[4]));
    } else if (specs[1] == "position") {
      m_LSetup->m_SpotLights[ParseInt(specs[0])].Position = Vector3f(
          ParseFloat(specs[2]), ParseFloat(specs[3]), ParseFloat(specs[4]));

    } else if (specs[1] == "direction") {
      m_LSetup->m_SpotLights[ParseInt(specs[0])].Direction =
          Vector3f(ParseFloat(specs[2]), ParseFloat(specs[3]),
                   ParseFloat(specs[4]))
              .Normalize();

    } else if (specs[1] == "attenuationlinear") {
      m_LSetup->m_SpotLights[ParseInt(specs[0])].Attenuation.Linear =
          ParseFloat(specs[2]);
