...
void CharacterAI::registerState(MainGameState *value) { state = value; }
...
void CharacterAI::setFacing() {
  state->activeCharacter->setAngle(
      getAngle(state->activeCharacter->getI(), state->activeCharacter->getJ(),
               attackLocation.getI(), attackLocation.getJ()));
}
...
void CharacterAI::performAction() {
  ...
  if (state->activeCharacter->getMoved() == false) {
    setDestination();
    performMove();
  } else if (state->activeCharacter->getAttacked() == false &&
...
void CharacterAI::setDestination() {
  ...
  HexCoordinate2D tempPos(state->activeCharacter->getI(),
  ...
  destination =
      HexCoordinate2D(strategyList.front().getI(), strategyList.front().getJ());
...
float CharacterAI::optimizeTechniqueAttack() {
  ...
  for (unsigned int T = 0; T < state->activeCharacter->getTechniques().size();
       T++) {
...
float CharacterAI::distanceToAllies(int i, int j) {
  ...
  for (auto ally : aiCharacters) {
    distance +=
        getDistance(state->activeCharacter->getI(),
                    state->activeCharacter->getJ(), ally->getI(), ally->getJ());
  }
...
void CharacterAI::setDifficulty(int value) { Difficulty = value; }

int CharacterAI::getDifficulty() { return Difficulty; }
