...
void CGameEngine::Init(const char* title, CGameState* initialState)
{
        ...
        while( std::getline(file, line) )
        {
            ...
            if ( line.compare(0,1,"#")!=0 )
            {
                ...
                if ( field.compare("width") == 0 )
                {
                    ...
                    ServiceLocator::getTextLog().addText("Width: "+ std::to_string(width));
                }
                ...
                else if ( field.compare("fullscreen") == 0)
                {
                    ...
                    //ServiceLocator::getTextLog().addText("Fullscreen: %d\n",fullscreen);
                }
            }
        }
    ...
    ServiceLocator::getGUISystem().subscribeFunction("MainMenu", "Quit", [this]()
    {
        this->Quit();
    });
    ServiceLocator::getGUISystem().subscribeFunction("MainMenu", "NewGame", [this]()
    {
        this->PushState(MainGameState::Instance());
    });
    ServiceLocator::getGUISystem().subscribeFunction("MainMenu", "MapEditor", [this]()
    {
        this->PushState(MapEditorState::Instance());
    });
...
CGameState* CGameEngine::GetCurrentState()
{
    return states.back();
}
...
{
    ServiceLocator::getGUISystem().injectDeltaTime(m_MainGameTimer.getSecondsElapsed());
...
void CGameEngine::Draw()
{
    ...
    ServiceLocator::getMainDisplay().getShaderManager()->setUniform(Pipeline::getInstance()->GetWorldTrans(),"gWorld");
    ServiceLocator::getMainDisplay().getShaderManager()->setUniform(Pipeline::getInstance()->getCameraTarget(),"cameraDirection");
