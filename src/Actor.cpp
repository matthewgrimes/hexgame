...
void Actor::Render()
{
    ...
    Pipeline::getInstance()->setObjectPos(Vector3f(Position.getX(), Position.getZ(), Position.getY()));
...
void Actor::Update()
{
    ...
    if (!moveList.empty())
    {
         ...
         *
         * */
        if ( m_MotionTimer.getSecondsElapsed() >= .5  && Angle != moveList.back() * 60 )
        {
