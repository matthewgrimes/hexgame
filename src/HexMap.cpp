...
void HexMapFileLoader::parseLine(const std::string &field,
  ...
  if (field == "size") {
    ...
    hexmap->m_LightingSetup.loadFromFile(std::string("data/maps/lighting/") +
                                         data);
  }
}
...
void HexMap::moveViewToPosition(int i, int j, unsigned int layer) {
  Pipeline::getInstance()->setObjectPos(
      Vector3f(2 * HORZDIST * i + HORZDIST * j, TILETHICKNESS * 1.f * layer,
               VERTDIST * j));
}
...
void HexMap::drawTile(int i, int j, unsigned int layer) {
  ...
  glUniform1f(glGetUniformLocation(ShaderManager::getInstance()->getProgram(),
                                   "waveTime"),
              m_TileAnimationTimer.getSecondsElapsed());
  glUniform1i(glGetUniformLocation(ShaderManager::getInstance()->getProgram(),
                                   "isWater"),
              currentTileset.getMember(tileType)->getImpassable());
  glUniform1i(glGetUniformLocation(ShaderManager::getInstance()->getProgram(),
                                   "drawGrid"),
              gridOn && (tileType != 0) &&
  ...
  if (cursorData[i][j] != 0 && layer == (unsigned int)getHeight(i, j) &&
    ...
    ResourceManager::getSingleton()
        ->getTexture(currentTileset.getAuxTileTexture(floor(cursorData[i][j])))
        ->Bind(GL_TEXTURE2);
  ...
  // if the tile is occupied by the cursor, draw that
    ...
    ShaderManager::getInstance()->setUniform(2, "gOverrideSampler");
    ResourceManager::getSingleton()
        ->getTexture(currentTileset.getAuxTileTexture(0))
        ->Bind(GL_TEXTURE2);
    currentTileset.getMember(tileType)->Draw();
    ShaderManager::getInstance()->setUniform(false, "gOverrideTexture");
  }
...
void HexMap::Draw() {
  ...
  while (iterator.Iterate(i, j)) {
    for (unsigned int layer = 0; layer <= (unsigned int)getHeight(i, j);
         layer++) {
      drawTile(i, j, layer);
    }
...
HexMap::drawMoveCircle(int i, int j, int r, int maxjump) {
...
HexMap::drawFilledCircle(int i0, int j0, int radius, bool actorsok,
                         int maxheightdiff, int color, bool colorGradient,
                         bool terrain) {
  std::deque<std::pair<int, int>> Frontier;
  ...
  while (!Frontier.empty()) {
    ...
    for (int d = 0; d < 6; d++) {
      ...
      if ((floor(cursorData[newI][newJ]) != color ||
          ...
          (!terrain ||
           !(currentTileset.getMember(getCurrentData(newI, newJ))
                 ->getImpassable()))) {
        ...
        cursorData[newI][newJ] =
            color +
            (int)colorGradient * newDistance * 1.f / (float)(radius + 1);
      }
    }
    Frontier.pop_front();
  }
...
void HexMap::clearDrawings() { cursorData.clear(); }
...
HexCoordinate3D HexMap::getMouseTile() {
  ...
  if (Pixel.gLayer > 0) {
    ...
    Return.setI(
        floor(Pixel.gI * 20 - (Return.getJ() - Return.getJ() % 2) / 2 + 0.5));
...
int HexMap::getCurrentTileSetSize() { return currentTileset.getSize(); }

Tileset *HexMap::getTileset() { return &currentTileset; }
  ...
  while (iterator.Iterate(i, j)) {
    ...
    for (unsigned int d = 0; d < 6; d++) {
      ...
      if (getCurrentData(newI, newJ, layer) != Data) {
        tileTransitionData[i][j] =
            tileTransitionData[i][j] | DirectionsBitFlag(d);
      }
    }
  }
}
