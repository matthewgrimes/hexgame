...
void TilesetLoader::parseLine(const std::string& field, const std::string& data)
{
    ...
    else
    {
        ...
        static_cast<HexTileMesh*>(tileset->currentTileSet.back())->LoadMesh(mesh.c_str());
        static_cast<HexTileMesh*>(tileset->currentTileSet.back())->LoadNormal(normal.c_str());

        tileset->currentTileSet.back()->setImpassable(atoi(data.substr(spacemarker+1).c_str()));
    }
}
...
void Tileset::Init()
{
    ...
    auxTileTextures = {"data/tiletextures/blue.png","data/tiletextures/red.png","data/tiletextures/green.png","data/tiletextures/white.png"};
...
Mesh* Tileset::getObstacle(int index)
{
    return obstacleSet[index];
}

Mesh* Tileset::getDecoration(int index)
{
