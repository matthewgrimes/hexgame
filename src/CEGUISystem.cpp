...
void CEGUISystem::Initialize() {
  CEGUI::OpenGL3Renderer &CEGUITempRenderer =
...
void CEGUISystem::HandleEvent(SDL_Event event) {
  ...
  case SDL_MOUSEMOTION:
    ...
    context.injectMousePosition(static_cast<float>(event.motion.x),
...
void CEGUISystem::subscribeFunction(std::string windowName,
  ...
  rootWindow->getChild(windowName)
      ->subscribeEvent(CEGUI::PushButton::EventClicked, CEGUIfunction);
}

void CEGUISystem::subscribeFunction(std::string parentName,
  ...
  rootWindow->getChild(parentName)
      ->getChild(windowName)
      ->subscribeEvent(CEGUI::PushButton::EventClicked, CEGUIfunction);
}
...
void CEGUISystem::set_CEGUI_paths() {
  ...
  defaultResProvider.setResourceGroupDirectory("schemes", "data/schemes/");

  defaultResProvider.setResourceGroupDirectory("imagesets", "data/imagesets/");

  defaultResProvider.setResourceGroupDirectory("fonts", "data/fonts/");

  defaultResProvider.setResourceGroupDirectory("layouts", "data/layouts/");

  defaultResProvider.setResourceGroupDirectory("looknfeels", "data/looknfeel/");

  defaultResProvider.setResourceGroupDirectory("lua_scripts",
                                               "data/lua_scripts/");

  defaultResProvider.setResourceGroupDirectory("schemas", "data/xml_schemas/");
...
void CEGUISystem::setProperty(const std::string &windowName,
...
/*void CEGUISystem::subscribeFunction(void * window, std::function<void(void)>
function)
{
        auto CEGUIfunction = [function](const CEGUI::EventArgs& e) { function();
return true; };
