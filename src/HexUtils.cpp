...
{
    return ( std::abs(i1-i2) + std::abs(j1-j2) + std::abs(i1+j1-i2-j2) )/2;
}
...
int HexCoordinate2D::getI() const
{
    return I;
}
int HexCoordinate2D::getJ() const
{
    return J;
}
float HexCoordinate2D::getX() const
{
    return X;
}
float HexCoordinate2D::getY() const
{
    return Y;
}
...
HexCoordinate3D::HexCoordinate3D(float x, float y, float z) : HexCoordinate2D(x, y)
{
    Z = z;
    Level = Z / TILETHICKNESS;
}

HexCoordinate3D::HexCoordinate3D(int i, int j, int level) : HexCoordinate2D(i, j)
{
    Level = level;
...
int HexCoordinate3D::getLevel()
{
    return Level;
}

float HexCoordinate3D::getZ()
{
    return Z;
}
...
/*
HexCoordinate3D convertScreenPositionToScene(float angle, int centerI, int centerJ, int windowX, int windowY, int Z)
{
    ...
    gluUnProject((GLdouble)winX, (GLdouble)winY, (GLdouble)winZ, modelview, projection, viewport, &sceneX, &sceneY, &sceneZ);
    GLdouble sceneX2, sceneY2, sceneZ2;
    gluUnProject((GLdouble)winX, (GLdouble)winY, (GLdouble)10., modelview, projection, viewport, &sceneX2, &sceneY2, &sceneZ2);
    ...
    //
    //
    // now we have are ready to calculate coordinates if the map weren't rotated.
