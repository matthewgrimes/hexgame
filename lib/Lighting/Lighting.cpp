...
bool LightingTechnique::Init() {
  ...
  for (unsigned int i = 0;
       i < sizeof(m_pointLightsLocation) / sizeof(m_pointLightsLocation[0]);
       i++) {
    char Name[128];
  ...
  for (unsigned int i = 0;
       i < sizeof(m_spotLightsLocation) / sizeof(m_spotLightsLocation[0]);
       i++) {
    char Name[128];
    memset(Name, 0, sizeof(Name));
    ...
    SNPRINTF(Name, sizeof(Name), "gSpotLights[%d].Base.Base.AmbientIntensity",
             i);
    ...
    SNPRINTF(Name, sizeof(Name), "gSpotLights[%d].Base.Base.DiffuseIntensity",
             i);
...
void LightingTechnique::bindDirectionalLight(const DirectionalLight &light) {
  ...
  glUniform3f(m_dirLightLocation.Direction, light.Direction.x,
              light.Direction.y, light.Direction.z);
  glUniform1f(m_dirLightLocation.DiffuseIntensity, light.DiffuseIntensity);
}
void LightingTechnique::bindPointLight(unsigned int i,
                                       const PointLight *pointLight) {
  glUniform3f(m_pointLightsLocation[i].Color, pointLight->Color.x,
              pointLight->Color.y, pointLight->Color.z);
  glUniform1f(m_pointLightsLocation[i].AmbientIntensity,
              pointLight->AmbientIntensity);
  glUniform1f(m_pointLightsLocation[i].DiffuseIntensity,
              pointLight->DiffuseIntensity);
  glUniform3f(m_pointLightsLocation[i].Position, pointLight->Position.x,
              pointLight->Position.y, pointLight->Position.z);
  glUniform1f(m_pointLightsLocation[i].Atten.Constant,
              pointLight->Attenuation.Constant);
  glUniform1f(m_pointLightsLocation[i].Atten.Linear,
              pointLight->Attenuation.Linear);
  glUniform1f(m_pointLightsLocation[i].Atten.Exp, pointLight->Attenuation.Exp);
}
void LightingTechnique::bindPointLights(unsigned int numLights,
...
void LightingTechnique::bindSpotLight(unsigned int i,
                                      const SpotLight *spotLight) {
  glUniform3f(m_spotLightsLocation[i].Color, spotLight->Color.x,
              spotLight->Color.y, spotLight->Color.z);
  glUniform1f(m_spotLightsLocation[i].AmbientIntensity,
              spotLight->AmbientIntensity);
  glUniform1f(m_spotLightsLocation[i].DiffuseIntensity,
              spotLight->DiffuseIntensity);
  glUniform3f(m_spotLightsLocation[i].Position, spotLight->Position.x,
              spotLight->Position.y, spotLight->Position.z);
  glUniform3f(m_spotLightsLocation[i].Direction, spotLight->Direction.x,
              spotLight->Direction.y, spotLight->Direction.z);
  glUniform1f(m_spotLightsLocation[i].Cutoff,
              cosf(ToRadian(spotLight->Cutoff)));
  glUniform1f(m_spotLightsLocation[i].Atten.Constant,
              spotLight->Attenuation.Constant);
  glUniform1f(m_spotLightsLocation[i].Atten.Linear,
...
void LightingTechnique::bindSpotLights(unsigned int numLights,
...
void LightingTechnique::bindPointLights(
...
void LightingTechnique::bindSpotLights(
