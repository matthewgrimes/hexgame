...
public:
    ...
    void setCurrentData(int i, int j, int value)
    {
        Data.front()[i][j] = value;
    }
    //! Method for working with back layer
    int getCurrentData(int i, int j)
    {
    ...
    //! Method for working with any layer
    int getCurrentData(int i, int j, int layer)
    {
        return Data[layer][i][j];
    }

    //! Get the height at a position
    unsigned int getHeight(int i, int j);

    std::map< std::pair<int, int>, std::vector<Directions> > drawMoveCircle(int i, int j, int r, int maxjump);
    ...
    std::map< std::pair<int, int>, int> drawFilledCircle(int i0, int j0, int radius, bool actorsok, int maxheightdiff, int color, bool colorGradient=true, bool terrain=true);
    void drawRing(int i0, int j0, int radius0, int radius1, int color);

    float getCursorData(int i, int j)
    {
        return cursorData[i][j];
    }
    ...
    void setCursorData(int i, int j, int value)
    {
        cursorData[i][j] = value;
    }

    void setCursorData( std::unordered_map< int, std::unordered_map<int, float> > value )
    {
        cursorData = value;
    }
    ...
    void addActorLocation(int i, int j)
    {
        actorLocations[i][j] = 1;
    }
    void clearActorLocations()
    {
...
private:
    ...
    std::unordered_map< int, std::unordered_map<int, int> > tileTransitionData; /**< the content of (i,j) is a bit flag corresponding to which neighbors are DIFFERENT */
    void initTileTransitionData(); /**< initialize the transition data once the map is loaded.*/
    ...
    void drawDecorationAndObstacle(const int i, const int j, const unsigned int layer);
