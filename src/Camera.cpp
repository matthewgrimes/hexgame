...
{
    rightVector = (Pipeline::getInstance()->getCameraTarget().Cross(Pipeline::getInstance()->getCameraUp())).Normalize();
    ...
    targetInterceptPlane *= Pipeline::getInstance()->getCameraPos().y / sinf(angleFromHorz(targetVector));
...
void Camera::changeZoom(float factor)
{
    if ( Zoom*factor > zoomMin && Zoom*factor < zoomMax )
    {
        ...
        //Pipeline::getInstance()->setFOV(Pipeline::getInstance()->getFOV() * factor );
