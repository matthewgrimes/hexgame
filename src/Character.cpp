...
void CharacterFileLoader::parseLine(const std::string& field, const std::string& data)
{
    ...
    else if ( field.compare("model") == 0)
    {
        ...
        character->m_PortraitHealthBar.LoadMesh("data/meshes/actorportraithealthbar.obj");
...
void Character::executeMove(int i, int j)
{
    ...
    while( current != std::make_pair(Position.getI(),Position.getJ()) )
    {
        ...
        current = std::make_pair(
                      NeighborI(current.first, current.second, direction),
...
void Character::addItem(Item* value)
{
    Inventory.push_back(value);
}

void Character::setWeapon(Weapon* value)
{
    currentWeapon = value;
}
...
void Character::performAttack(Character* target)
{
    ...
    outputString = getName() + std::string(" is attacking ") + target->getName() + std::string(" with ") + getCurrentAttack()->getName() + std::string(":");
...
void Character::printStatus()
{
    ...
    output = "HP: " + std::to_string(getHitPoints()) + "/" + std::to_string(getMaxHitPoints());
    ServiceLocator::getTextLog().addText(output);
    ...
    output = "Range: " + std::to_string(getCurrentWeapon()->getMinRange()) + "-" + std::to_string(getCurrentWeapon()->getMaxRange());
    ServiceLocator::getTextLog().addText(output);
...
void Character::RenderPortrait()
{
    ...
    Pipeline::getInstance()->setObjectScale(Vector3f(1, 1, getHitPointsPercent()));
    Pipeline::getInstance()->setObjectPos( Vector3f(objectPos.x, 0.f, objectPos.z - (1-getHitPointsPercent()) ) );
